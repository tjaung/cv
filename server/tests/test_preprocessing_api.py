...
class PreprocessingPipelineTests(unittest.TestCase):
    def test_distinct_steps_and_segmentation_only_patch_preview(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            gray = raw[:, :, 0]
            result = SimpleNamespace(normalized=raw, gray=gray, blurred=gray, edge_bold=gray,
            ...
            with patch('server.api.os_helpers.DATASET_ROOT', root):
                ...
                for pipeline, count, last in [('segmentation', 10, 'Patch extraction'), ('full', 16, 'Contrast / final plate')]:
                ...
                with patch('server.api.preprocessing.segment_plate', return_value=result), \
                    ...
                    status, body = get({'pipeline': 'segmentation', 'step': 8})
                    self.assertEqual(status, 200)
                ...
                self.assertEqual(get({'pipeline': 'segmentation', 'step': 10})[0], 422)
