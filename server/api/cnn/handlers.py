...
from ..shared.os_helpers import folder_path, image_path

ARTIFACT_ROOT = Path(__file__).resolve().parents[3] / 'artifacts/cnn'
...
def _input_signature():
    ...
    import hashlib
    root = Path(__file__).resolve().parents[3] / 'CV'
...
def test_cnn(body: RunRequest):
    ...
    def run_cached(job_id, data):
        ...
        if summary['config'].get('variant') in ('patch', 'patch_multiclass'):
            from .patches import test_patch_run
