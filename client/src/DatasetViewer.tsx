...
function GroundTruthToggle({ opacity, onChange, disabled = false, grid = false }: {
  opacity: number
  ...
  return <div className="overlay-controls">
    <label className="cropped-option"><input type="checkbox" checked={opacity > 0} disabled={disabled}
      onChange={(event) => onChange(event.target.checked ? 50 : 0)} />
      {grid ? 'Show ground truth · all images' : 'Show ground truth'}
    </label>
  </div>
}
...
function TestImagePreview({ imageSet, name, opacity, onOpacityChange }: {
  ...
  return <>
    <GroundTruthToggle opacity={opacity} onChange={onOpacityChange} disabled={!ready || !!error} />
...
function Directory({ path, sets, navigate }: {
  ...
  return <div className="workspace">
    ...
    <section className="panel preview" aria-label="Image preview">
      ...
      {view === 'grid' ? <>
        <GroundTruthToggle opacity={gridOpacity} onChange={setGridOpacity} disabled={!canOverlayGrid} grid />
