import { ClassifiersWriteup } from './ClassifiersWriteup'
...
export type Model = { id: string; model_saved?: boolean; model_available?: boolean; config: { kind: string; variance_target: number; [key: string]: string | number }; cv: Record<string, { mean: number; std: number; folds?: number[] }>; evaluation: Evaluation; training_evaluation?: Evaluation }
...
export default function Classifiers() {
  ...
  return <section><div className="page-intro model-writeup-intro"><h2>Classifiers</h2><ClassifiersWriteup /></div>
    ...
    {!summary ? <p className="panel empty">Train classifiers to create a shared split and comparisons.</p> : <>
      ...
      <section className="panel"><div className="panel-heading"><h3>Classifier comparison</h3><span>{rows.length} models</span></div><p className="feature-note">Default ranking uses cross-validation macro F1, not holdout scores. CV values are mean ± standard deviation. Gaps are training minus holdout in percentage points (pp). Click a model to inspect its metrics and predictions.</p><div className="model-table-scroll"><table><thead><tr>{header('name','Model')}{header('cv','CV macro F1')}{header('train_accuracy','Training accuracy')}{header('accuracy','Holdout accuracy')}{header('accuracy_gap','Accuracy gap')}{header('f1_gap','Macro-F1 gap')}{header('balanced','Balanced accuracy')}{header('f1','Macro F1')}{summary.classes.map(c => header(c,c))}<th>Keep fitted model</th></tr></thead><tbody>{rows.map(m => <tr key={m.id} aria-selected={model?.id===m.id}><th><button onClick={() => setChoice(m.id)}>{name(m)}</button></th><td>{pct(m.cv.macro_f1.mean)} ± {pct(m.cv.macro_f1.std)}</td><td>{m.training_evaluation ? pct(m.training_evaluation.accuracy) : '—'}</td><td>{pct(m.evaluation.accuracy)}</td><td>{gap(m.training_evaluation?.accuracy,m.evaluation.accuracy)}</td><td>{gap(m.training_evaluation?.report['macro avg']['f1-score'],m.evaluation.report['macro avg']['f1-score'])}</td><td>{pct(m.evaluation.balanced_accuracy)}</td><td>{pct(m.evaluation.report['macro avg']['f1-score'])}</td>{summary.classes.map(c=>{const r=m.evaluation.report[c];return <td key={c}>{Math.round(r.recall*r.support)}/{r.support} ({pct(r.recall)})</td>})}<td><SaveModelButton family="classifiers" runId={summary.run_id} modelId={m.id} saved={m.model_saved} available={m.model_available} /></td></tr>)}</tbody></table></div></section>
      ...
      {model && <section className="panel model-training"><h3>Evaluation · {name(model)}</h3><Generalization model={model} /><ClassifierLearningCurves key={`${summary.run_id}:${model.id}`} run={summary.run_id} model={model.id} canCompute={model.model_available !== false} /><p>Balanced accuracy averages class recall. Macro F1 weights classes equally; weighted F1 accounts for class frequency. Weighted F1: {pct(model.evaluation.report['weighted avg']['f1-score'])}.</p><div className="model-composition-grid"><div className="model-table-scroll"><table><thead><tr><th>Class</th><th>Precision</th><th>Recall</th><th>F1</th><th>Support</th></tr></thead><tbody>{summary.classes.map(c=>{const r=model.evaluation.report[c];return <tr key={c}><th>{c}</th><td>{pct(r.precision)}</td><td>{pct(r.recall)}</td><td>{pct(r['f1-score'])}</td><td>{r.support}</td></tr>})}</tbody></table></div><div className="model-table-scroll"><table><caption>Confusion matrix · rows actual, columns predicted</caption><thead><tr><th>Actual / predicted</th>{model.evaluation.classes.map(c=><th key={c}>{c}</th>)}</tr></thead><tbody>{model.evaluation.confusion_matrix.map((r,i)=><tr key={i}><th>{model.evaluation.classes[i]}</th>{r.map((n,j)=><td key={j} style={{ background: i===j ? '#e1f0e6' : n ? '#fce2df' : undefined }}>{n}</td>)}</tr>)}</tbody></table></div></div></section>}
      {sample && <section className="panel"><div className="panel-heading"><h3>Individual holdout predictions</h3><div className="model-controls"><button disabled={testIndex===0} onClick={()=>setIndex(testIndex-1)}>←</button><select aria-label="Holdout image" value={testIndex} onChange={e=>setIndex(Number(e.target.value))}>{summary.test.map((s,i)=><option key={s.path} value={i}>{s.path}</option>)}</select><button disabled={testIndex===summary.test.length-1} onClick={()=>setIndex(testIndex+1)}>→</button></div></div><p className="feature-note">{model?.id===choice ? <>Following {name(model)} · pinned first with automatic patch explanations. <button onClick={() => setChoice('')}>Stop following</button></> : 'Click a model name to follow it across images and automatically explain its predicted defect patches.'}</p><div className="classifier-inspection"><figure>{model ? <ClassifierPatchImage key={`${model.id}:${testIndex}`} autoExplain={model.id === choice} run={summary.run_id} model={model.id} index={summary.train.length+testIndex} src={getImageUrl('metal_plate', sample.original_split, sample.path.split('/').slice(2).join('/'))} alt={sample.path} /> : <img src={getImageUrl('metal_plate', sample.original_split, sample.path.split('/').slice(2).join('/'))} alt={sample.path}/>}<figcaption>{sample.path.split('/').pop()} · actual: {sample.label}{model && <p className={model.evaluation.predictions[testIndex]===sample.label ? 'classification-right' : 'classification-wrong'}>{model.evaluation.predictions[testIndex]===sample.label ? 'Right' : 'Wrong'} · predicted: {model.evaluation.predictions[testIndex]}</p>}</figcaption></figure><div className="model-table-scroll"><table><thead><tr><th>Model</th><th>Prediction</th><th>Correct</th></tr></thead><tbody>{[...rows].sort((a,b)=>Number(b.id===choice)-Number(a.id===choice) || Number(b.evaluation.predictions[testIndex]===sample.label)-Number(a.evaluation.predictions[testIndex]===sample.label)).map(m=><tr key={m.id} aria-selected={model?.id===m.id}><th><button aria-pressed={choice===m.id} onClick={()=>setChoice(m.id)}>{choice===m.id ? 'Following · ' : ''}{name(m)}</button></th><td>{m.evaluation.predictions[testIndex]}</td><td className={m.evaluation.predictions[testIndex]===sample.label?'classification-right':'classification-wrong'}>{m.evaluation.predictions[testIndex]===sample.label?'Right':'Wrong'}</td></tr>)}</tbody></table></div>{model && <Space key={`${model.id}:${testIndex}`} model={model} index={testIndex} classes={summary.classes}/>}</div></section>}
