import colorHistograms from '../../../../images/image-1.png'
import grayscaleHistograms from '../../../../images/image.png'
    ...
    <article className="preprocessing-writeup-content">
      ...
      <div className="writeup-pipelines">
        ...
        <section><h3>Pre Process the plate</h3><ol>
          ...
          <li>CLAHE (Enhances local contrast in small regions, which may make faint defects more visible but can also amplify noise.)</li>
      ...
      <p>{"The models use different parts of this pipeline. The full pipeline with manually created features is for the classical models for anomaly detection and classification. The CNN approach only uses up until the segmented out plate, followed by the ResNet input transforms. The reason for this is because I wanted to experiment with CNN to see what kind of features it would learn."}</p>
      <p>{"I came to this pipeline after some trial and error."}</p>
      <p>{"I initially had an idea of normalizing colors to the intensity midpoint. Given a percentage, pixels below the midpoint increase in luminance by that percentage, capped at the midpoint. Brighter pixels decrease in luminance, also stopping at the midpoint. I initially thought this did not make much of a difference from looking at the images and histograms, so I removed it. After retraining, I found that the anomaly model lost a lot of its scratch detection, while both whole plate CNNs improved on the inspected evaluation set and the classifier results were mixed. I then compared the normalization choices for anomaly detection, as shown in that tab. It has options for the full pipeline with or without luminance normalization, and raw images with features only. Classifiers and CNNs still use their existing pipelines without luminance correction."}</p>
