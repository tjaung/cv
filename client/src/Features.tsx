...
const channelNames: Record<HistogramChannel, string> = { L: 'L* · lightness', a: 'a* · green → red', b: 'b* · blue → yellow', H: 'H · hue', S: 'S · saturation', V: 'V · brightness', magnitude: 'Sobel magnitude · edge strength', orientation: 'Sobel orientation · weighted by strength', lbp: 'Uniform LBP · texture patterns' }
const channelUnits: Record<HistogramChannel, string> = { L: 'L* value', a: 'a* value', b: 'b* value', H: 'Hue (degrees)', S: 'Saturation (%)', V: 'Value (%)', magnitude: 'Intensity change / pixel', orientation: 'Gradient orientation (degrees)', lbp: 'LBP code (9 = non-uniform)' }
...
function FeatureImage({ path, source }: { path: string; source: FeatureSource }) {
  ...
  return <div className="panel feature-panel">
    ...
    <div className="feature-source-images"><figure><img src={data.image} alt={`${source} source ${data.name}`} /><figcaption>{source === 'original' ? 'Original color image' : 'Repaired color plate (before blur / CLAHE)'}</figcaption></figure>
...
function HistogramPlot({ data, channel, hidden, region }: { data: FeatureHistograms; channel: HistogramChannel; hidden: string[]; region?: number }) {
  ...
  return <article className="lab-chart"><h3>{region === undefined ? channelNames[channel] : `Row ${Math.floor(region / 4) + 1}, column ${region % 4 + 1}`}</h3>
    <svg viewBox="0 0 600 275" role="img" aria-label={`${channelNames[channel]} normalized histograms by class`} onMouseLeave={() => setHover(null)} onMouseMove={(event) => {
      ...
      {channel === 'lbp' ? Array.from({ length: 10 }, (_, i) => <text key={i} x={x(i)} y="242" textAnchor="middle">{i}</text>) : [0, .25, .5, .75, 1].map((fraction) => <text key={fraction} x={54 + fraction * 520} y="242" textAnchor="middle">{(edges[0] + fraction * (edges[bins] - edges[0])).toFixed(0)}</text>)}
      <text x="314" y="264" textAnchor="middle">{channelUnits[channel]}</text><text x="54" y="24">{channel === 'orientation' ? 'Gradient strength per bin (%)' : 'Eligible pixels per bin (%)'}</text>
...
export default function Features() {
  ...
  return <section>
    <div className="page-intro"><p className="eyebrow">METAL PLATE · FEATURE EXPLORATION</p><h2>Features</h2>
      <p>Explore CIELAB lightness and color channels, plus Sobel gradients, magnitude, and direction. Compare original color values with the repaired color plate from preprocessing. Both modes use the same extracted plate region to exclude background.</p>
    </div>
    <div className="panel feature-toolbar"><div className="view-switch" aria-label="Feature source">{(['original', 'preprocessed'] as const).map((value) => <button key={value} aria-pressed={source === value} onClick={() => setSource(value)}>{value === 'original' ? 'Original images' : 'Preprocessed plates'}</button>)}</div>
      ...
      {error ? <p role="alert">{error} <button onClick={() => { setError(''); setAttempt(attempt + 1) }}>Retry</button></p> : <label className="feature-image-select">Image <select value={selected} onChange={(event) => setSelected(event.target.value)} disabled={!images.length}>{images.map((path) => <option key={path} value={path}>{path.replace('metal_plate/', '')}</option>)}</select></label>}
      <div className="view-switch"><button disabled={index <= 0} onClick={() => setSelected(images[index - 1])}>← Previous</button><button disabled={index < 0 || index >= images.length - 1} onClick={() => setSelected(images[index + 1])}>Next →</button><span>{images.length ? `${index + 1} / ${images.length}` : 'Loading images…'}</span></div>
    </div>
    {selected && <FeatureImage key={`${selected}:${source}`} path={selected} source={source} />}
